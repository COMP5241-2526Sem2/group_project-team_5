...
class QuizIllustrationService:
        ...
        *,
        ...
        async with semaphore:
            ...
            for attempt in range(1, max_attempts + 1):
                try:
                    provider = settings.illustration_provider.strip().lower()
                    if provider in {"openai", "ohmygpt"}:
                        png_bytes, image_prompt = await asyncio.wait_for(
                        ...
                        image_url = "data:image/svg+xml;base64," + base64.b64encode(svg_bytes).decode("ascii")
                        image_prompt = "[non-llm provider fallback svg]"

                    QuizIllustrationService._dump_trace(
    ...
    def _get_client() -> AsyncOpenAI:
        if QuizIllustrationService._client is None:
            key = (settings.ohmygpt_api_key or "").strip() or (settings.gpt_api_key or "").strip()
            if not key:
                raise RuntimeError("Missing API key for illustration generation")
            base_url = (settings.illustration_base_url or "").strip() or settings.ohmygpt_base_url
    ...
    async def _generate_openai_image(
        *,
        ...
        model = (settings.illustration_model or "").strip() or "gpt-image-1"
        ...
        if not response.data or not response.data[0].b64_json:
            raise RuntimeError("EmptyImageResponse")
        return base64.b64decode(response.data[0].b64_json), full_prompt

    @staticmethod
    def _dump_trace(*, question_id: str, stage: str, payload: dict) -> None:
        if not settings.dump_llm_raw_sessions:
            return
        try:
            root = Path(__file__).resolve().parents[3]
            out_dir = root / "data" / "llm_illustration_traces"
            out_dir.mkdir(parents=True, exist_ok=True)
            ts = datetime.utcnow().strftime("%Y%m%d_%H%M%S_%f")
            out = out_dir / f"{ts}_{stage}_{question_id}.json"
            out.write_text(json.dumps(payload, ensure_ascii=False, indent=2), encoding="utf-8")
        except Exception as exc:
            logger.warning("Failed to dump illustration trace: %s", repr(exc))
