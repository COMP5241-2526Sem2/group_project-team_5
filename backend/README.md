...
如果前端出现接口 `HTTP 500` 或代理 `ECONNREFUSED`，建议先在后端目录执行：
...
- `RESULT: PASS|FAIL`
- `CAUSE: DNS|PORT|AUTH|SSL|...`

可选参数：

```bash
python scripts/check_db_preflight.py --timeout 8
python scripts/check_db_preflight.py --dsn "postgresql+asyncpg://user:pass@host:6543/postgres"
```
